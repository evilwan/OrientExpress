...
#
# External command to use to interact with ExpressVPN
#
EXPRESSVPN_CMD = kodi.setting("expressvpncmd")
FOEFEL = True


def log_debug(msg):
    if FOEFEL:
        kodi.log("orientexpress.py --: {}".format(msg))


log_debug("EXPRESSVPN_CMD: {}".format(EXPRESSVPN_CMD))
...
def do_vpn_command(prog, args):
    ...
    """
    command = [prog] + args
    log_debug("do_vpn_command() -- command = {}".format(command))
    p = subprocess.Popen(
        command,
    ...
    res = stdout.decode()
    log_debug("do_vpn_command() -- result=\n{}\n".format(res))
    return res
...
def is_running():
    ...
    try:
        ...
        log_debug("is_running() -- caught: {}".format(err))
...
def disconnect():
    ...
    try:
        ...
        log_debug("disconnect() -- caught: {}".format(err))
...
def connect():
    ...
    try:
        ...
        log_debug("connect() -- caught: {}".format(err))
...
def connect_specific(exit_node):
    ...
    try:
        ...
        log_debug("connect_specific() -- caught: {}".format(err))
        raise ExpressvpnError(3, "Unable to connect Expressvpn")
...
def list_all_exitnodes():
    ...
    try:
        ...
        log_debug("list_all_exitnodes() -- caught: {}".format(err))
